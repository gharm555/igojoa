        ...
        .then((response) => {
            if (response.data === true) {
                if (email.length >= 20) {
                    ...
                    $emailFeedback.textContent = "변경 가능한 이메일입니다.";
                    $emailFeedback.style.color = "green";
                }
            } else {
                $emailFeedback.textContent = "중복된 이메일입니다.";
                $emailFeedback.style.color = "red";
            }
        })
        .catch((error) => {
...
function checkPhoneNumber() {
    ...
    if (phone1.length === 3 && phone2.length === 4 && phone3.length === 4) {
        axios
            ...
            .then((response) => {
                if (response.data === true) {
                    $phoneFeedback.textContent = "사용 가능한 전화번호입니다.";
                    $phoneFeedback.style.color = "green";
                } else {
                    $phoneFeedback.textContent = "이미 등록된 전화번호입니다.";
