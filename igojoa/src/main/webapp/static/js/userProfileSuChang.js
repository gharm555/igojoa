...
function levelColor(level) {
  console.log(level); //출력 ㄴㄴ 안나ㅗ아여 ㅋㅋㅋㅋㅋㅋ
  if (level >= 10) return { bg: orange };
  if (level >= 20) return { bg: yellow };
  if (level >= 30) return { bg: red };
  if (level >= 40) return { bg: red };
  if (level >= 50) return { bg: red };
  if (level >= 60) return { bg: red };
  if (level >= 70) return { bg: red };
  if (level >= 80) return { bg: red };
  if (level >= 90) return { bg: red };
  return { bg: "red" };
}
function getLevel() {
  const points = document.querySelector(".cumulativePoints").innerHTML;
  console.log(points); // 이건 출력 됩니다요 숫자찍혀요? 8900나와여 ㅋㅋㅋ 아 근데 숫자만 나오는게 아니고 <span> ㅁㄴㅇㅁㄴㅇㅁㄴㅇ</span>
//8900
  return Math.floor(points / 1000) + 1; //그럼 얘가 문젠데 1000포인트당 1레벨, 시작 레벨은 1
}
levelIcon.innerHTML = getLevel();
levelIcon.style.backgroundColor = levelColor(getLevel()).bg;
